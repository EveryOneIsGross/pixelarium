...
function selectMemoryWeightedMove(worm, moves) {
    ...
    const scoredMoves = moves.map(move => {
        ...
        // Special bias: if worm is at very bottom with no material support, strongly prefer climbing up
        if (worm.head.y === GRID_HEIGHT - 1) {
            // Check if there's actual material support nearby
            let hasMaterialSupport = false;
            for (let dy = -1; dy <= 1; dy++) {
                for (let dx = -1; dx <= 1; dx++) {
                    if (dx === 0 && dy === 0) continue;
                    const checkX = worm.head.x + dx;
                    ...
                    if (isValidPos(checkX, checkY)) {
                        ...
                        if (cell === SAND || cell === DIRT || cell === GLASS ||
                            cell === TREE || cell === DEAD_TREE || cell === WATER) {
                            hasMaterialSupport = true;
                            break;
                        }
                    }
                }
                if (hasMaterialSupport) break;
            }

            // If no material support and move goes upward, give massive bonus
            if (!hasMaterialSupport && move.dy < 0) {
                score += 5.0; // Strong preference to climb up
            }
        }
