...
function updateTreeBehavior() {
    // Rebuild tree list every 30 frames
    if (treeUpdateCounter % 30 === 0) {
        treesToProcess = [];
        treeProcessIndex = 0;
    ...
    // Process a batch of trees each frame (amortized processing)
    if (treesToProcess.length === 0) return;

    const endIndex = Math.min(treeProcessIndex + treesPerFrame, treesToProcess.length);
    ...
    // Reset index when we've processed all trees
