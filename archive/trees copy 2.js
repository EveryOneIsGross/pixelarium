...
function updateTreeBehavior() {
    // Rebuild tree list every 3 frames for more responsive growth (was 30)
    if (treeUpdateCounter % 3 === 0) {
        treesToProcess = [];
        // DON'T reset treeProcessIndex - maintain round-robin position across rebuilds
    ...
    // Process ALL trees each frame instead of batching (fixes incomplete coverage)
    ...
    // No more batch tracking - all trees processed each frame
}
